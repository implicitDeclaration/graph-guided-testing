...
def compute_average_degree(G):
    ...
    return ave_degree
def random_regular_graph(neighbors,nodes):
    # RG = nx.Graph()
    # for i in range(nodes):
    #     RG.add_edge(i,(i+1)%nodes)
    # for i in range(0,nodes,4):
    #     RG.add_edge(i%nodes,(i+2)%nodes)
    #     RG.add_edge((i+1)%nodes,(i+3)%nodes)
    # return RG
    while True:
        try:
            print('==>begin to generate Regular Graph')
            sequence = np.ones(nodes)*neighbors
...
def Generate_Optimal_Symmetric_Network(neighbors, nodes):

    if not args.whether_search:
        RG = random_regular_graph(neighbors, nodes)
        edges = np.array(RG.edges())
